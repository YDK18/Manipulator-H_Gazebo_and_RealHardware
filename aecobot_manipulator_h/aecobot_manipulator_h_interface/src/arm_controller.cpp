...
#include "aecobot_manipulator_h_interface/arm_controller.h"

using namespace aecobot_manipulator_h;
...
{
	aecobot_manipulator_h_msgs::JointPosition msg;
...
void ArmController::initPublisher(bool using_gazebo)
{
  if (using_gazebo)
  {
    ...
    std::string joint_name[joint_num_] = {"joint1", "joint2", "joint3", "joint4", "joint5", "joint6"};

    for (uint8_t index = 0; index < joint_num_; index++)
    {
      if (robot_name_ == "aecobot_manipulator_h")
      {
  ...
  arm_state_pub_ = nh_.advertise<aecobot_manipulator_h_msgs::State>(robot_name_ + "/arm_state", 10);
}
...
bool ArmController::getJointPositionMsgCallback(aecobot_manipulator_h_msgs::GetJointPosition::Request &req,
...
bool ArmController::getKinematicsPoseMsgCallback(aecobot_manipulator_h_msgs::GetKinematicsPose::Request &req,
...
bool ArmController::setJointPositionMsgCallback(aecobot_manipulator_h_msgs::SetJointPosition::Request &req,
		aecobot_manipulator_h_msgs::SetJointPosition::Response &res)
{
  aecobot_manipulator_h_msgs::JointPosition msg = req.joint_position;
  res.isPlanned = calcPlannedPath(msg);
}

bool ArmController::setKinematicsPoseMsgCallback(aecobot_manipulator_h_msgs::SetKinematicsPose::Request &req,
		aecobot_manipulator_h_msgs::SetKinematicsPose::Response &res)
{
  aecobot_manipulator_h_msgs::KinematicsPose msg = req.kinematics_pose;
  res.isPlanned = calcPlannedPath(msg);
}

bool ArmController::calcPlannedPath(aecobot_manipulator_h_msgs::KinematicsPose msg)
{
  ros::AsyncSpinner spinner(1);
  spinner.start();
...
bool ArmController::calcPlannedPath(aecobot_manipulator_h_msgs::JointPosition msg)
{
  ros::AsyncSpinner spinner(1);
  spinner.start();
...
void ArmController::process(void)
{
  ...
  aecobot_manipulator_h_msgs::State state;
...
{
  ros::init(argc, argv, "joint_controller_for_AECOBOT_Manipulator_H");
