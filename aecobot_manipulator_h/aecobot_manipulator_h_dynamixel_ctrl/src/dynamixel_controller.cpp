...
#include "aecobot_manipulator_h_dynamixel_ctrl/dynamixel_controller.h"
...
DynamixelController::DynamixelController()
    :node_handle_(""),
     priv_node_handle_("~")
{
  robot_name_   = priv_node_handle_.param<std::string>("robot_name", "aecobot_manipulator_h");
  ...
  //gripper_id_.push_back(priv_node_handle_.param<int>("gripper_id", 7));
...
//  gripper_controller_->begin(device_name.c_str(), dxl_baud_rate);
  ...
  ROS_INFO("aecobot_manipulator_h_dynamixel_controller : Init OK!");
}
...
//  joint_state.name.push_back("grip_joint_sub");
  ...
  joint_states_pos[4] = get_joint_position[4];
  joint_states_pos[5] = get_joint_position[5];
  joint_states_pos[6] = joint_states_pos[5];
...
//  joint_states_pos[5] = joint_states_pos[4];
  ...
  joint_states_vel[5] = get_joint_velocity[5];
  joint_states_vel[6] = joint_states_vel[5];
//  joint_states_vel[4] = get_joint_velocity[4];
...
{
  double goal_joint_position[JOINT_NUM] = {0.0, };
...
{
  // Init ROS node
  ros::init(argc, argv, "aecobot_manipulator_h_dynamixel_controller");
