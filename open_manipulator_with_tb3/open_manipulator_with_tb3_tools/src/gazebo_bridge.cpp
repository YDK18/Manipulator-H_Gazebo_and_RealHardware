...
//ros::Subscriber joint_trajectory_point_sub_, joint6_position_sub_;
...
/*void joint6PositionCallback(const std_msgs::Float64MultiArray::ConstPtr& msg)
{
...
{
  const uint8_t POINT_SIZE = 6 + 1; //Joint num + Time
...
/*  pb = nh.advertise<std_msgs::Float64>("gripper_position/command", 10);
  ...
  pb = nh.advertise<std_msgs::Float64>("joint5_position/command", 10);
  gazebo_goal_joint_position_pub_.push_back(pb);

  pb = nh.advertise<std_msgs::Float64>("joint6_position/command", 10);
  gazebo_goal_joint_position_pub_.push_back(pb);
...
{  
  joint_trajectory_point_sub_ = nh.subscribe("joint_trajectory_point", 1000, jointTrajectoryPointCallback);
  joint6_position_sub_ = nh.subscribe("gripper_position", 10, joint6PositionCallback);
}*/
